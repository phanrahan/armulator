...
#define DPRegRHS  ((BITS (4,11) == 0) ? state->Reg[RHSReg] : GetDPRegRHS (state, instr))
#define DPSRegRHS ((BITS (4,11) == 0) ? state->Reg[RHSReg] : GetDPSRegRHS (state, instr))

#define LSBase state->Reg[LHSReg]
#define LSImmRHS (BITS(0,11))

#define LSRegRHS ((BITS (4, 11) == 0) ? state->Reg[RHSReg] : GetLSRegRHS (state, instr))

#define LSMNumRegs ((ARMword) ARMul_BitList[BITS (0, 7)] + (ARMword) ARMul_BitList[BITS (8, 15)] )
#define LSMBaseFirst ((LHSReg == 0 && BIT (0)) || (BIT (LHSReg) && BITS (0, LHSReg - 1) == 0))
...
#define BYTETOBUS(data) ((data & 0xff) | ((data & 0xff) << 8) | ((data & 0xff) << 16) | ((data & 0xff) << 24))
